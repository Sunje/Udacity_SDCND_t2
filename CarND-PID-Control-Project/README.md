...
* Simply summarizing the method, if the `cte` exceeds the reference value (line 69 in [main.cpp][main.cpp]), the update procedure is executed. After the update, if the `cte` becomes smaller than the reference value, the update procedure stops. Otherwise, the update procedure is executed according to the condition. Only one hyperparemeter is updated at a time.
